...
- **Composer libraries blocked from web access.** The root `htaccess` answered direct requests for files under `vendor/`, so `vendor/composer/installed.json` disclosed every library and its exact version, and library PHP files could be requested directly. `htaccess` now forbids `vendor/`, and the bundled nginx config denies `/vendor/` and `/application/`. Existing Apache installs pick this up by replacing `.htaccess` with the new `htaccess`; nginx installs should add the same two `location` blocks.
...
[@0xMoError-22](https://github.com/0xMoError-22),
...
| CSRF on seven `delete()`-style endpoints missing token validation | Medium | [Missing CSRF Token Validation on Multiple Delete Endpoints](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-9372-vj68-hmc3) | [@Char0n1507](https://github.com/Char0n1507), [@vicrypter](https://github.com/vicrypter) | [#1637](https://github.com/InvoicePlane/InvoicePlane/pull/1637) |
...
| Privilege revocation failure: downgraded administrator retains administrative access via stale session | High | [Privilege Revocation Failure: Downgraded Administrator Retains Administrative Access (Persistent Privilege Escalation)](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-XXXX-XXXX-XXXX) | [@0xMoError-22](https://github.com/0xMoError-22) | [#1681](https://github.com/InvoicePlane/InvoicePlane/pull/1681) |
| PayPal `order_id` path injection: unvalidated order_id interpolated into the PayPal API request path in `captureOrder()`/`showOrderDetails()` | Low | [Limited SSRF / Path Traversal in PayPal Gateway](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-rf9w-hm5h-4gr2) | [@Char0n1507](https://github.com/Char0n1507) | [#1684](https://github.com/InvoicePlane/InvoicePlane/pull/1684) |
| Recurring-invoice cron race condition (TOCTOU): concurrent cron runs could double-generate the same recurring invoice and duplicate the customer email | Medium | [Race Condition in Recurring Invoice Cron](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-hvvh-fjx5-9j55) | [@Char0n1507](https://github.com/Char0n1507) | [#1684](https://github.com/InvoicePlane/InvoicePlane/pull/1684) |
| No rate limiting on wrong `cron_key` attempts against the unauthenticated cron endpoint | Medium | [Cron Key Weak PRNG and No Rate Limiting](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-28fh-2656-wc63) | [@Char0n1507](https://github.com/Char0n1507) | [#1684](https://github.com/InvoicePlane/InvoicePlane/pull/1684) |
| Login IP-based rate limiting was session-scoped, so omitting the session cookie reset the counter on every request | High | [Session-Stored IP Rate Limiting Bypass](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-hcxg-3465-jc8m) | [@Char0n1507](https://github.com/Char0n1507) | [#1684](https://github.com/InvoicePlane/InvoicePlane/pull/1684) |
| `Sessions::authenticate()` was a `public` method, directly routable via a bare GET request (`/sessions/sessions/authenticate/<email>/<password>`), bypassing the login form, CSRF, and POST entirely | High | [Unauthenticated Session Creation via Public Sessions::authenticate() Endpoint Leading to Full Admin Access](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-x2m4-962x-cphw), [Unauthenticated Session Creation via Sessions::authenticate() Endpoint](https://github.com/InvoicePlane/InvoicePlane/security/advisories/GHSA-mwjf-mrp3-8r4x) (duplicate) | [@Char0n1507](https://github.com/Char0n1507) | [#1684](https://github.com/InvoicePlane/InvoicePlane/pull/1684) |
...
- [#1681](https://github.com/InvoicePlane/InvoicePlane/pull/1681) — **Privilege revocation failure:** `User_Controller::__construct()` now re-validates the acting user's `user_type` and `user_active` status from the authoritative `ip_users` record on **every** request, destroying the session if the role no longer matches the required value or the account is deactivated. `Mdl_sessions::invalidate_user_sessions()` destroys all active sessions for a user when their role or active status is changed, forcing immediate revocation of stale sessions. `Users::form()` prevents users from escalating their own role and only explicitly sets `user_type` from POST in authorized scenarios (new user creation, admin editing another user with role change). Previously, a downgraded administrator retained administrative access through a stale session and could restore their own role via POST, achieving persistent privilege escalation.
